...
/* ------------------------------------------------------------
   ข้อความ "รู้ตัวไหมว่า..." — จุดที่ควรทำให้ผู้ใช้รู้สึกว่าตัวเองเก่งกว่าที่คิด
   ไม่ใช่แค่โชว์ตัวเลขเฉยๆ (Stage 2 ของ empathy journey) แบ่งเป็น 3 ระดับ
   ตามความครอบคลุมจริง — ไม่ปั้นคำชมเกินสิ่งที่ตัวเลขรองรับ
   ------------------------------------------------------------ */
export function coverageRevealHeadline(cov: number | null, roleName: string): string {
  if (cov === null) return "";
  if (cov >= 70) return `ว้าว! คุณพร้อมเป็น ${roleName} ไปแล้ว ${cov}% — เก่งกว่าที่คิดใช่ไหมล่ะ`;
  if (cov >= 40) return `คุณรู้ตัวไหมว่าตอนนี้พร้อมเป็น ${roleName} ไปแล้ว ${cov}% — ใกล้กว่าที่คิด`;
  return `ตอนนี้คุณพร้อมเป็น ${roleName} ไปแล้ว ${cov}% — ดูเส้นทางที่เหลือด้านล่างได้เลย`;
}
